...
#define SCAL      0x33   // Calibrate frequency synthesizer and then turn it off
#define SRX       0x34   // Enable RX (calibrates if coming from IDLE)
#define STX       0x35   // Enable TX (if in IDLE; in RX+CCA, go to TX only if clear)
...
void rfInitT(void)
{
    ...
    */
    ...
    RFST = SFRX;  // Flush RX FIFO
    RFST = SFTX;  // Flush TX FIFO
